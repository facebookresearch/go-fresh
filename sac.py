...
class SAC(object):
    def __init__(self, cfg, space_info, device, log):
        ...
        self.critic = QNetwork(space_info['shape']['obs'], 
                space_info['action_dim'], cfg.policy)
        self.critic = self.critic.to(device=self.device)
        self.critic_optim = Adam(self.critic.parameters(), lr=cfg.optim.lr)

        self.critic_target = QNetwork(space_info['shape']['obs'],
                space_info['action_dim'], cfg.policy)
        ...
        if self.cfg.policy.type == "Gaussian":
            ...
            if self.cfg.optim.entropy.auto_tuning is True:
                self.target_entropy = - torch.prod(torch.Tensor((space_info['action_dim'],)).to(self.device)).item()
                self.log_alpha = torch.zeros(1, requires_grad=True,
                        device=self.device)
                self.alpha_optim = Adam([self.log_alpha],
                        lr=cfg.optim.entropy.lr)

            self.policy = GaussianPolicy(space_info['shape']['obs'],
                    space_info['action_dim'], cfg.policy).to(self.device)

        else:
            self.alpha = 0
            self.cfg.optim.entropy.auto_tuning = False
            self.policy = DeterministicPolicy(space_info['shape']['obs'],
                    space_info['action_dim'], cfg.policy).to(self.device)
